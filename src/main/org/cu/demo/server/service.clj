(ns org.cu.demo.server.service
  (:require
    ...
    [org.cu.demo.server.pathom-parser :refer [parser]]
...
(defn make-csp-policy
  ...
  (let [{:keys [csp dev?]} (::config config)
    ...
    {:default-src "'self'"
     ...
     :script-src  (str "'self' 'unsafe-inline'" (when dev? " 'unsafe-eval'") " https://accounts.google.com/")
