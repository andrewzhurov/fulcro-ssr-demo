(ns org.cu.demo.task.task-resolvers
  (:require
    ...
    [org.cu.demo.task.db-layer :as dl]
...
(pc/defmutation create-task-mutation
  ...
  {::pc/sym 'org.cu.demo.task/create-task}
...
(pc/defresolver all-tasks [{:keys [xtdb-node]} _]
  {::pc/output [{:all-tasks dm/db-task-keys}]}
  (log/info "all-tasks resolver")
  (log/spy {:all-tasks (dl/get-all-tasks xtdb-node)}))
