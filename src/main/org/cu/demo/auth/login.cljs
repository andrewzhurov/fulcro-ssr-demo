...
(defsc Session
  ...
  {:query         [:session/valid? :session/server-error-msg :user/id :user/name :user/email :user/picture :ui/loading?]
    ...
    (dom/form :.ui.form
      ...
      (dom/div :.ui.message
        ...
        (dom/a {:onClick (fn []
                           ...
                           (comment #_fr/change-route! this :signup))}
          "Please sign up!")))))
...
(defsc Login [this {:user/keys [email]
  ...
  {:query              [:ui/open? :ui/error :user/email
                        session-join
                        [::sm/asm-id :org.cu.demo.client.ui.root/TopRouter]
          ...
          [:a.item
           {:key      "signup"
            :on-click (fu/prevent-default (fn []
                                            ...
                                            (comment #_fr/change-route! this :signup)))
            :href     (if (fu/on-server?) "signup" (comment #_fr/route-href :signup))}
           "Signup"]
