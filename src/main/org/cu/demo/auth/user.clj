...
(defn make-user [id email password name picture]
  (cond-> {:user/id    id
           :user/name  name
           :user/email email
           :user/tasks []
           :user/picture picture}
    password (assoc :user/password (hash-password password))))

(defn insert-user [id email password name picture]
  (cu/insert-entity xtdb-node :user/id (make-user id email password name picture)))

(defn insert-user-map [m]
  (cu/insert-entity xtdb-node :user/id m))
...
(defn get-user-by-email [username]
  (cu/entity-with-prop xtdb-node [:user/email username]))

(defn get-user-by-id [xtdb-node id]
  (cu/entity xtdb-node id))

(defn get-all-users []
  (->>
    (cu/crux-select xtdb-node [:user/id :user/email])
    (map #(cu/domain-entity xtdb-node (:user/id %)))
    (group-by :user/email)))
  ...
  (when-let [session (:session request)]
    (when (:session/valid? session)
      (if-let [email (:user/email session)]
        (do (log/info "Have a user email: " email)
...
(defresolver user-resolver [{:keys [xtdb-node]} {:user/keys [id]}]
  {::pc/input  #{:user/id}
   ::pc/output [:user/email]}
  (select-keys (get-user-by-id xtdb-node id) [:user/email]))
...
(pc/defresolver all-users-resolver [env _]
  ...
  {:all-users (map (comp #(cu/domain-entity xtdb-node %) :user/id)
                (cu/crux-select xtdb-node [:user/id]))})
