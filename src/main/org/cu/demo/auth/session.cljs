...
(defn logout [env]
  (let [env
    ...
    (comment #_fr/change-route! (::sm/app env) :default)
    env))

(defn login [{::sm/keys [event-data] :as env}]
  (-> env
    (clear)
    (sm/trigger-remote-mutation :actor/login-form 'org.cu.demo.auth.session/login
...
(defn process-session-result
  ...
  (let [success? (sm/alias-value env :session-valid?)]
    (cond
      ...
      (comment #_fr/change-route! (::sm/app env) :default)
      (not success?)
      (comment #_fr/change-route! (::sm/app env) :signup))
    (cond-> (clear env)
      success? (->
