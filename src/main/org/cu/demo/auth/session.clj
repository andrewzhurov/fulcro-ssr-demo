...
(pc/defmutation signup
  ...
  {::pc/sym 'org.cu.demo.auth.signup/signup}
  (let [id (fu/uuid)]
    (if-let [user (user/get-user-by-email email)]
      ...
      (do (log/info "doing signup")
          ...
          (user/insert-user id email password email nil)
          (augment-session-resp env {:session/valid? true :session/server-error-msg nil :user/name email})))))

(pc/defmutation auth-google
  [env {:keys [jwt]}]
  {::pc/sym 'org.cu.demo.auth.signup/auth-google}
  (let [json                      (log/spy (jwt/str->jwt jwt))
        {google-name    :name
         google-email   :email
         google-picture :picture} (:claims json)]
    ;; todo only insert if different
    (if-let [{:user/keys [id]} (log/spy (user/get-user-by-email google-email))]
      (do (user/insert-user id google-email nil google-name google-picture)
          (let [{:user/keys [id name email picture] :as user} (log/spy (user/get-user-by-email google-email))]
            (log/spy user)
            (augment-session-resp env {:session/valid? true :session/server-error-msg nil :user/id id :user/name name :user/email email :user/picture picture})))))
  #_(let [id (fu/uuid)]
      (if-let [user (user/get-user-by-email email)]
        (augment-session-resp env
                              {:session/valid? false :user/name nil :session/server-error-msg "Problem signing up."})
...
(def resolvers [current-session-resolver signup login logout auth-google])
